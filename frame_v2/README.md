...
For the current Linux local setup, use [LOCAL_RUNBOOK.md](LOCAL_RUNBOOK.md).
It documents the verified start command, persistence, troubleshooting, and the
security limitations of the checked-in Compose configuration. The deployment
sections below contain historical macOS, DNS, Nginx, and Kubernetes notes; they
do not all match the current `docker-compose.yml`.
