...
def extract_m4a_album(file_path):
    try:
        with open(file_path, 'rb') as f:
            res = find_mp4_atom(f, [b'moov', b'udta', b'meta', b'ilst', b'\xa9alb'])
            if res:
                start, size = res
                res_data = find_mp4_atom(f, [b'data'], start, size)
                if res_data:
                    data_start, data_size = res_data
                    f.seek(data_start + 16)
                    album_data = f.read(data_size - 16)
                    return album_data.decode('utf-8', errors='ignore').replace('\x00', '').strip()
    except Exception as e:
        print(f"Error extracting M4A album: {e}")
    return None

def extract_mp3_album(file_path):
    try:
        with open(file_path, 'rb') as f:
            header = f.read(10)
            if len(header) < 10 or header[0:3] != b'ID3':
        ...
        print(f"Error extracting MP3 album: {e}")
    return None
...
def init_db():
    with get_db() as conn:
        with conn.cursor() as cur:
            ...
            # Self-healing routine to extract missing artwork/artist/album for existing tracks
            try:
                cur.execute("""
                    SELECT id, storage_name, mime, artwork_url, artist, album
                    FROM music_tracks
                    WHERE artwork_url IS NULL OR artwork_url = '' OR artist IS NULL OR artist = '' OR album IS NULL OR album = ''
                """)
                missing_tracks = cur.fetchall()
                if missing_tracks:
                    print(f"Found {len(missing_tracks)} existing tracks with missing artwork, artist, or album. Starting extraction...")
                    for track_id, storage_name, mime, old_art, old_artist, old_album in missing_tracks:
                        dest_path = os.path.join(FILE_UPLOAD_DIR, storage_name)
                        if not os.path.exists(dest_path):
                            continue
                        
                        ext = os.path.splitext(storage_name)[1].lower() or ""
                        artwork_url = old_art
                        artist = old_artist
                        album = old_album
                        
                        art_res = None
                        if not old_art or old_art == '':
                            if mime == "audio/mpeg" or ext == ".mp3":
                                art_res = extract_mp3_artwork(dest_path)
                            elif mime in ["audio/mp4", "audio/x-m4a"] or ext in [".m4a", ".mp4"]:
                            ...
                            if art_res:
                                ...
                                artwork_url = clean_db_string(f"/static/uploads/artwork/{art_name}")
                        
                        if not old_artist or old_artist == '':
                            if mime == "audio/mpeg" or ext == ".mp3":
                        ...
                        if not old_album or old_album == '':
                            if mime == "audio/mpeg" or ext == ".mp3":
                                album = extract_mp3_album(dest_path)
                            elif mime in ["audio/mp4", "audio/x-m4a"] or ext in [".m4a", ".mp4"]:
                                album = extract_m4a_album(dest_path)
                            if album:
                                album = clean_db_string(album)
                                
                        if artwork_url != old_art or artist != old_artist or album != old_album:
                            cur.execute("""
                                UPDATE music_tracks
                                SET artwork_url = %s, artist = %s, album = %s
                                WHERE id = %s
                            """, (artwork_url, artist, album, track_id))
                    conn.commit()
                    print("Existing tracks self-healing artwork/artist/album extraction finished.")
            except Exception as ex:
                print(f"Error in music tracks artwork/album self-healing routine: {ex}")

            # Clean up absolute artwork URLs to relative path format to prevent domain/SSL mismatch issues
            try:
                cur.execute("""
                    UPDATE music_tracks
                    SET artwork_url = '/static/uploads/artwork/' || split_part(artwork_url, '/artwork/', 2)
                    WHERE artwork_url LIKE '%/artwork/%' AND artwork_url LIKE 'http%';
                """)
                updated_count = cur.rowcount
                conn.commit()
                if updated_count > 0:
                    print(f"Successfully converted {updated_count} absolute artwork URLs to relative paths.")
            except Exception as ex:
                print(f"Error converting absolute artwork URLs to relative: {ex}")
...
def upload_file():
    ...
    # Extract artwork, artist, and album if possible
    artwork_url = None
    artist = None
    album = None
    try:
        ...
        print(f"Error extracting artwork/artist/album for {storage_name}: {e}")
        ...
        with conn.cursor() as cur:
            cur.execute("""
                INSERT INTO music_tracks (
                    id, storage_name, original_name, url, download_url, size, mime, doc_name, artwork_url, artist, album
                ) VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s)
            """, (
                track_id, storage_name, safe_name, public_url, download_endpoint, size, mimetype, doc_name, artwork_url, artist, album
            ))
            conn.commit()
...
def list_music_tracks():
    ...
    with get_db() as conn:
        with conn.cursor() as cur:
            cur.execute("""
                SELECT id, original_name, storage_name, size, mime, uploaded_at, artwork_url, artist, album
                FROM music_tracks
                WHERE doc_name = %s
